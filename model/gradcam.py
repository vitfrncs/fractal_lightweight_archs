...
def get_target_layers(model):
    ...
    # ConvNeXtV2 (usa model.features para os estágios e blocos)
    if hasattr(model, "features") and not isinstance(model.features, nn.Sequential):
        # Para estruturas do torchvision convnext, features é um nn.Sequential contendo CNBlockStages
        if len(model.features) > 0:
            return [model.features[-1]] # Último estágio convolucional
    ...
    # Fallback genérico para capturar a última convolução caso mude a arquitetura
    last_conv = None
